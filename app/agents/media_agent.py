...
class MediaAgent(BaseAgent):
    def __init__(self, llm: LLMClient):
        ...
        role_prompt = (
            ...
            "你的任务是：专门从短视频平台、社交媒体或新闻网站中提取视觉画面信息和大众评论。\n"
            "你拥有以下强大的工具：\n"
            "1. `web_search`：用于搜索互联网资讯，获取网页链接。\n"
            "2. `scrape_webpage`：用于抓取指定网页的正文和提取图片链接。\n"
            "3. `analyze_image`：用于“看懂”图片链接中的画面内容。\n\n"
            "【强制行动指南】：\n"
            "- 第一步：使用 `web_search` 获取目标新闻或社媒网页的 URL。\n"
            "- 第二步：调用 `scrape_webpage` 抓取该 URL 的内容。\n"
            "- 第三步：**关键！** 如果 `scrape_webpage` 抓取结果中包含【发现关键图片链接】，你必须立即调用 `analyze_image` 工具，将图片 URL 传给它，以获取画面视觉分析。\n"
